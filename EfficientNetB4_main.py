...
import os
import datetime
from EfficientNetB4_builder import build_efficientnet_model # ⬅️ 수정된 빌더 파일 import
...
def main():
    ...
    print(f"\n--- 2. EfficientNet 모델 생성 중 ({num_classes} 클래스)... ---")
    
    # Eager Execution 강제 활성화 (NotImplementedError 방지)
    ...
    if os.path.exists(MODEL_SAVE_PATH):
        ...
        # 이전 학습 에포크 수 수동 지정 (필요 시 수정)
        initial_epoch = 20
        
        # ⚠️ 로드 후 재컴파일: Eager Execution 및 학습률 적용을 위해 필수
