...
def set_custom_style(background_image_base64):
    ...
    st.markdown(
        ...
        /* 제목 텍스트 스타일 */
        h1 {{color: #ffffff; text-shadow: 2px 2px 5px #000000; text-align: center; padding-top: 50px; padding-bottom: 50px; font-size: 3.5em; margin-bottom: 0;}}
        
        /* 메인 콘텐츠 영역의 배경을 반투명하게 설정 */
        .block-container {{ background-color: rgba(255, 255, 255, 0.95); padding: 20px; border-radius: 10px; margin-top: 20px;}}
        
        /* 기타 스타일 */
        .stImage > img {{border: 2px solid #008080; border-radius: 10px; box-shadow: 2px 2px 10px rgba(0, 0, 0, 0.2);}}
        </style>
        """,
...
def load_all_models_and_prep():
    ...
    models = OrderedDict()
    custom_loss = {'focal_loss': lambda y_true, y_pred: tf.reduce_mean(-y_true * tf.math.log(y_pred))} 
    ...
    if os.path.exists(CONVNEXT_MODEL_PATH):
        try:
            ...
            # ⬇️ [핵심] ConvNeXt 로드 실패 시 경고만 띄우고 시스템은 계속 진행
            st.warning(f"⚠️ ConvNeXt 로드 실패 (호환성 오류): 시스템이 로드되지 않았습니다.", icon="⚠️")
...
def predict_and_format_result(model_data, img_file):
    ...
    try:
        ...
        # ConvNeXt 오류 발생 시, 예측 실패로 처리하고 시스템 중단 방지
        return None, 0, "⚠️ 예측 실패", pd.DataFrame({'버섯 종류': ['오류'], '확신도 (%)': [0]})

    score = tf.nn.softmax(predictions[0]).numpy() # numpy 변환
    
    # 3. TOP 3 확신도 로직
    class_probabilities = list(zip(CLASS_NAMES, score)) # numpy() 제거
...
if all_models:
    ...
    if uploaded_file is not None:
        ...
        with col_img:
            ...
            # use_container_width=True로 설정하여 경고 제거 및 컨테이너 너비에 맞게 조정
            ...
            for idx, (model_name, model_data) in enumerate(all_models.items()):
                ...
                with result_columns[idx]:
                    ...
                    with st.spinner(f"{model_name} 예측 중 (입력: {model_data['size'][0]}x{model_data['size'][1]})..."):
                        ...
                        if top_name and top_name != "오류": # 예측 성공 시에만 결과 출력
